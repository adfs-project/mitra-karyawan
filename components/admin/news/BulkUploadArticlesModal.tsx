...
// FIX: Replaced useApp with useData to use the new unified data context.
...
const BulkUploadArticlesModal: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
    ...
    const handleDownloadTemplate = () => {
        const headers = ['title', 'summary', 'content', 'category', 'status (Published/Draft)', 'type (standard/poll/qa/Banner)', 'imageUrl', 'youtubeId'];
        const ws = XLSX.utils.json_to_sheet([{}], { header: headers });
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Template Artikel");
        XLSX.writeFile(wb, "template_artikel.xlsx");
    };
    ...
    const validateData = (data: any[]) => {
        ...
        const validTypes = ['standard', 'poll', 'qa', 'Banner'];

        for (const row of data) {
            if (!row.title || !row.summary || !row.content || !row.category) {
                validatedRows.push({ data: row, status: 'invalid', error: 'Kolom wajib (title, summary, content, category) harus diisi.' });
                continue;
            }
            if (row['status (Published/Draft)'] && !validStatuses.includes(row['status (Published/Draft)'])) {
                validatedRows.push({ data: row, status: 'invalid', error: 'Status tidak valid.' });
                continue;
            }
            if (row['type (standard/poll/qa/Banner)'] && !validTypes.includes(row['type (standard/poll/qa/Banner)'])) {
                validatedRows.push({ data: row, status: 'invalid', error: 'Tipe tidak valid.' });
                continue;
            }
    ...
    const handleImport = async () => {
        ...
        setIsProcessing(true);
        const articlesToCreate = validRows.map(row => ({
            title: row.title,
            summary: row.summary,
            content: row.content,
            category: row.category,
            status: row['status (Published/Draft)'] || 'Published',
            type: row['type (standard/poll/qa/Banner)'] || 'standard',
            imageUrl: row.imageUrl,
            youtubeId: row.youtubeId,
        }));

        const result = await addMultipleArticlesByAdmin(articlesToCreate);
    ...
    const renderStepContent = () => {
        switch (step) {
            case 2:
                ...
                return (
                    <>
                        <h2 className="text-2xl font-bold text-primary mb-4">Pratinjau Data Artikel</h2>
                        ...
                        <div className="max-h-60 overflow-y-auto border border-border-color rounded-lg">
                            <table className="w-full text-xs text-left">
                                <thead className="bg-surface-light sticky top-0"><tr><th className="p-2">Judul</th><th className="p-2">Kategori</th><th className="p-2">Status</th></tr></thead>
                                <tbody>
                                    {previewData.map((row, i) => (
                                        <tr key={i} className={row.status === 'invalid' ? 'bg-red-500/10' : ''}>
                                            <td className="p-2 border-t border-border-color">{row.data.title}</td>
            ...
            case 3:
                return (
                    <div className="text-center">
                        ...
                        <p className="text-xl"><span className="font-bold text-green-400">{importResult?.success || 0}</span> artikel berhasil dibuat.</p>
            ...
            default:
                return (
                    <>
                        <h2 className="text-2xl font-bold text-primary mb-4">Unggah Artikel Massal</h2>
                        <div className="space-y-4">
                            ...
                            <p className="text-text-secondary">2. Isi data artikel pada templat (termasuk URL media), lalu unggah file di sini.</p>
                            <div className="relative">
                                <input type="file" id="bulk-article-upload-input" className="absolute w-0 h-0 opacity-0" onChange={handleFileChange} accept=".xlsx, .xls" />
                                <label htmlFor="bulk-article-upload-input" className="w-full flex items-center justify-center p-3 btn-secondary rounded-lg font-bold cursor-pointer">
...
export default BulkUploadArticlesModal;