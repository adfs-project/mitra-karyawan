...
const getTransactionIcon = (type: Transaction['type']) => {
    switch (type) {
        case 'Top-Up': case 'Refund': case 'Reversal': case 'Dana Opex': case 'Insurance Claim': return <ArrowUpCircleIcon className="h-8 w-8 text-green-400" />;
        default: return <ArrowDownCircleIcon className="h-8 w-8 text-red-400" />;
    }
};

const StickySidebar: React.FC<{ onTopUp: () => void, onTransfer: () => void }> = ({ onTopUp, onTransfer }) => {
    const { user } = useAuth();
    return (
        <div className="space-y-6">
            <div className="bg-surface p-6 rounded-lg shadow-lg border border-border-color text-center">
                 ...
                 <p className="text-4xl font-bold text-primary my-2"> {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(user?.wallet.balance || 0)} </p>
            </div>
            <PayLaterStatusCard />
            <div className="grid grid-cols-2 gap-4">
                 <button onClick={onTopUp} className="flex items-center justify-center space-x-2 p-3 bg-surface rounded-lg hover:bg-surface-light transition-colors font-semibold"> <ArrowUpCircleIcon className="h-6 w-6 text-primary" /> <span>Top Up</span> </button>
                 <button onClick={onTransfer} className="flex items-center justify-center space-x-2 p-3 bg-surface rounded-lg hover:bg-surface-light transition-colors font-semibold"> <ArrowsRightLeftIcon className="h-6 w-6 text-primary" /> <span>Transfer</span> </button>
            </div>
        </div>
    );
};
...
const WalletScreen: React.FC = () => {
    ...
    const userTransactions = useMemo(() => transactions .filter(tx => tx.userId === user?.id) .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()), [transactions, user?.id]);
    const filteredTransactions = useMemo(() => { if (filterType === 'All') { return userTransactions; } return userTransactions.filter(tx => tx.type === filterType); }, [userTransactions, filterType]);
    const renderContent = () => {
        switch (activeTab) {
            case 'Analitik': return <WalletAnalytics transactions={userTransactions} />;
            case 'Perencanaan': return <FinancialPlanning />;
            case 'Riwayat':
                return (
                            ...
                            <div key={tx.id} className="bg-surface p-4 rounded-lg flex items-center space-x-4">
                                ...
                                <div className={'font-bold text-right'}>
                                    <p className={`${tx.amount > 0 ? 'text-green-400' : 'text-red-400'}`}>{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(tx.amount)}</p>
                                     <span className={`text-xs font-normal ${tx.status === 'Completed' ? 'text-green-500' : tx.status === 'Pending' ? 'text-yellow-500' : 'text-red-500'}`}> {tx.status} </span>
                                </div>
                            </div>
    ...
    return (
        <div className="py-6 grid grid-cols-3 gap-8">
            ...
            <main className="col-span-3 lg:col-span-2 space-y-6">
                 <button onClick={() => setAdvisorModalOpen(true)} className="w-full btn-secondary p-3 rounded-lg flex items-center justify-center font-bold space-x-2"> <SparklesIcon className="h-6 w-6" /> <span>Tanya Analis Keuangan AI</span> </button>
                <div>
                    <div className="flex border-b border-border-color">
                        {(['Analitik', 'Perencanaan', 'Riwayat'] as Tab[]).map(tab => ( <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 font-semibold ${activeTab === tab ? 'text-primary border-b-2 border-primary' : 'text-text-secondary'}`}> {tab} </button> ))}
                    </div>
                    {activeTab === 'Riwayat' && (
                        <div className="py-2">
