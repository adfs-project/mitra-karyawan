...
const EditProfileModal: React.FC<{isOpen: boolean; onClose: () => void; user: User;}> = ({ isOpen, onClose, user }) => {
    ...
    const handleSave = () => {
        updateCurrentUser({ ...user, profile: { ...user.profile, name, phone, photoUrl } });
        onClose();
    };
                ...
                <div className="space-y-4">
                    <div className="flex justify-center">
                        <div className="relative">
                            <img src={photoUrl} alt="Profile" className="w-24 h-24 rounded-full border-2 border-primary object-cover" />
                            <button
                                onClick={() => fileInputRef.current?.click()}
                                className="absolute bottom-0 right-0 bg-primary text-black p-1.5 rounded-full hover:bg-primary/80"
