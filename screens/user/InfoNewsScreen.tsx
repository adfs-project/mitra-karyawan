        ...
        <div className="py-6 grid grid-cols-10 gap-8">
            <main className="col-span-10 lg:col-span-7 space-y-4">
                ...
                <div className="flex border-b border-border-color">
                    <button onClick={() => setActiveTab('For You')} className={`flex items-center space-x-2 px-4 py-2 font-semibold ${activeTab === 'For You' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary'}`} > <SparklesIcon className="h-5 w-5" /> <span>For You</span> </button>
                    <button onClick={() => setActiveTab('Global')} className={`flex items-center space-x-2 px-4 py-2 font-semibold ${activeTab === 'Global' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary'}`} > <GlobeAltIcon className="h-5 w-5" /> <span>Global</span> </button>
                </div>

                <div>
                    {activeTab === 'For You' ? ( <AiPersonalizedFeed onOpenComments={handleOpenComments} /> ) : ( <div className="space-y-4"> {globalFeed.map(article => ( <ArticleCard key={article.id} article={article} onOpenComments={() => handleOpenComments(article)} /> ))} </div> )}
                </div>
            </main>
