...
const PbbTaxScreen = () => {
    ...
    const provider = apiIntegrations.find(api => api.id === serviceLinkage['gov-pbb']);
    const isConnected = !!provider;

    const handleCheckBill = () => {
        if (!nop.trim()) {
            showToast('Nomor Objek Pajak (NOP) harus diisi.', 'warning');
            return;
        }
        if (provider) {
            showToast(`Memeriksa tagihan PBB untuk NOP ${nop} via ${provider.name}... (Simulasi)`, 'info');
        }
    };
        ...
        <div className="p-4 space-y-6">
            ...
            <div className="bg-surface p-6 rounded-lg border border-border-color">
                ...
                <p className="text-text-secondary mb-4">
                    {isConnected 
                        ? 'Bayar Pajak Bumi dan Bangunan Anda dengan mudah.'
                ...
                <div className={`space-y-4 ${!isConnected ? 'opacity-70' : ''}`}>
                    <div>
                        ...
                        <input 
                            ...
                            placeholder="e.g., 32.73.xxxx.xxx.xxx-xxxx.x" />
                    </div>
