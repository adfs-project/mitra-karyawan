...
const MpnG3Screen = () => {
    ...
    const provider = apiIntegrations.find(api => api.id === serviceLinkage['gov-mpn']);
    const isConnected = !!provider;

    const handlePay = () => {
        if (!billingCode.trim()) {
            showToast('Kode Billing harus diisi.', 'warning');
            return;
        }
        if (provider) {
            showToast(`Memproses pembayaran untuk Kode Billing ${billingCode} via ${provider.name}... (Simulasi)`, 'info');
        }
    };
        ...
        <div className="p-4 space-y-6">
            ...
            <div className="bg-surface p-6 rounded-lg border border-border-color">
                ...
                <p className="text-text-secondary mb-4">
                    {isConnected 
                        ? 'Bayar pajak, bea cukai, dan penerimaan negara lainnya.'
                ...
                <div className={`space-y-4 ${!isConnected ? 'opacity-70' : ''}`}>
                    <div>
                        ...
                        <input 
                            ...
                            placeholder="Masukkan 15 digit Kode Billing" />
                    </div>
                     <button onClick={handlePay} disabled={!isConnected} className={`w-full p-3 font-bold rounded-lg ${isConnected ? 'btn-primary' : 'bg-gray-600 cursor-not-allowed'}`}>
                        Lanjut Bayar
                    </button>
                </div>
            </div>
