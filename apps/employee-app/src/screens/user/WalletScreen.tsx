...
const getTransactionIcon = (type: Transaction['type']) => {
    switch (type) {
        case 'Top-Up': case 'Refund': case 'Reversal': case 'Dana Opex': case 'Insurance Claim':
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                 <button onClick={onTopUp} className="flex items-center justify-center space-x-2 p-3 bg-surface rounded-lg hover:bg-surface-light transition-colors font-semibold">
                    <ArrowUpCircleIcon className="h-6 w-6 text-primary" />
                    <span>Top Up</span>
                </button>
                 <button onClick={onTransfer} className="flex items-center justify-center space-x-2 p-3 bg-surface rounded-lg hover:bg-surface-light transition-colors font-semibold">
...
const WalletScreen: React.FC = () => {
    ...
    const filteredTransactions = useMemo(() => filterType === 'All' ? userTransactions : userTransactions.filter(tx => tx.type === filterType), [userTransactions, filterType]);
    ...
    return (
        <div className="lg:grid lg:grid-cols-3 lg:gap-8">
            ...
            {/* --- Main Content --- */}
            <main className="lg:col-span-2 p-4 lg:p-0 space-y-6">
                ...
                <div>
                    <div className="flex border-b border-border-color">
                        {(['Analitik', 'Perencanaan', 'Riwayat'] as Tab[]).map(tab => (
                            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 font-semibold ${activeTab === tab ? 'text-primary border-b-2 border-primary' : 'text-text-secondary'}`}>{tab}</button>
                        ))}
                    </div>
