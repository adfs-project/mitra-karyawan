...
import { useData, useAuth, Notification, Product } from '@mk/shared';
import { 
    BuildingStorefrontIcon, CurrencyDollarIcon, NewspaperIcon, ClockIcon, Squares2X2Icon, SparklesIcon,
    BellIcon
    ...
    return (
        <div>
            ...
            <div className={`grid ${gridColsClass} gap-4 text-center`}>
                {baseItems.map(item => (
                    <Link to={item.path} key={item.name} onClick={() => logEngagementEvent('quickAccessClicks', item.name)} className="bg-surface p-4 rounded-lg flex flex-col items-center justify-center aspect-square">
...
const AttendanceCard: React.FC = () => {
    ...
    const activeRecord = useMemo(() => {
        if (!user) return null;
        return [...(attendanceRecords || [])].filter(r => r.userId === user.id && r.clockInTime && !r.clockOutTime).sort((a, b) => new Date(b.clockInTime!).getTime() - new Date(a.clockInTime!).getTime())[0];
    ...
    const latestRecordToday = useMemo(() => {
        ...
        return recordsToday.sort((a, b) => {
            ...
            return timeB - a.getTime();
        })[0];
    }, [attendanceRecords, user]);
    const getStatus = () => {
        if (activeRecord) return { text: `Anda masuk pukul ${new Date(activeRecord.clockInTime!).toLocaleTimeString('id-ID')}.`, button: 'Clock Out' };
        if (latestRecordToday?.clockOutTime) return { text: `Absen terakhir hari ini pukul ${new Date(latestRecordToday.clockOutTime).toLocaleTimeString('id-ID')}.`, button: 'Clock In' };
    ...
    return (
        <>
            <div className="bg-surface p-4 rounded-lg border border-border-color">
                <h2 className="text-lg font-bold text-text-primary mb-2 flex items-center"><ClockIcon className="h-5 w-5 mr-2" /> Absensi Hari Ini</h2>
...
const RightSidebar: React.FC = () => {
    const { notifications, user, products } = useData();
    const userNotifications = (notifications || []).filter(n => n.userId === user?.id).slice(0, 3);
    const cooperativeProducts = (products || []).filter(p => p.sellerName === 'Koperasi Mitra').slice(0, 2);

    return (
        <div className="space-y-6">
            <div className="p-4 bg-surface rounded-lg border border-border-color">
                 <h3 className="font-bold text-text-secondary text-sm mb-2 flex items-center"><BellIcon className="h-4 w-4 mr-2"/> Notifikasi Terbaru</h3>
                 <div className="space-y-2">
                     ...
                     )) : <p className="text-xs text-text-secondary text-center">Tidak ada notifikasi.</p>}
                </div>
            </div>
            <div className="p-4 bg-surface rounded-lg border border-border-color">
                 <h3 className="font-bold text-text-secondary text-sm mb-2">Sorotan Koperasi</h3>
                 <div className="space-y-3">
                    {cooperativeProducts.map((p: Product) => (
                        <Link key={p.id} to="/market" className="flex items-center space-x-2 group">
                            <img src={p.imageUrl} alt={p.name} className="w-10 h-10 rounded-md object-cover"/>
                            <div>
                                <p className="text-xs font-semibold group-hover:text-primary">{p.name}</p>
                                <p className="text-xs text-primary font-bold">{new Intl.NumberFormat('id-ID', {style: 'currency', currency: 'IDR'}).format(p.price)}</p>
                            </div>
                        </Link>
                    ))}
                 </div>
