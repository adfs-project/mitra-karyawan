...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const updateApiIntegration = useCallback(async (id: string, creds: any): Promise<{ success: boolean; message: string }> => {
        const integrationIndex = state.apiIntegrations.findIndex(api => api.id === id);
        if (integrationIndex === -1) {
            const message = 'Integration not found.';
            showToast(message, 'error');
            return { success: false, message };
        }
        const integration = state.apiIntegrations[integrationIndex];
        ...
        if (result.success) {
            ...
            const updatedApis = [...state.apiIntegrations];
            updatedApis[integrationIndex] = { ...updatedApis[integrationIndex], status: IntegrationStatus.Active };
            updateState('apiIntegrations', updatedApis);
        } else {
            vaultService.updateApiStatus(id, IntegrationStatus.Error);
    
            const updatedApis = [...state.apiIntegrations];
            updatedApis[integrationIndex] = { ...updatedApis[integrationIndex], status: IntegrationStatus.Error };
            updateState('apiIntegrations', updatedApis);
        }
    
        return result;
    }, [state.apiIntegrations, updateState, showToast]);
